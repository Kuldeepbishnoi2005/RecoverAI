...
async def get_recovery_results(transaction_id: str):
    """
    Returns recovery simulation results for a transaction.
    """
    try:
        supabase = get_supabase_admin_client()
        res = supabase.table("recovery_actions").select("*").eq("transaction_id", transaction_id).order("created_at", desc=True).execute()
        if res.data:
            return res.data
    except Exception:
        pass
